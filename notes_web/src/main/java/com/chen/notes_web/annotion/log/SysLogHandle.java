...
import com.chen.notes_web.entity.WebVisit;
...
public class SysLogHandle extends AbstractRequestAwareRunnable {
    ...
    protected void onRun() {
        ...
        webVisit.setIp(ip);
        //从Redis中获取IP来源
        String jsonResult = redisUtil.get(RedisConf.IP_SOURCE + RedisConf.SYMBOL_COLON + ip);
        if (StrUtil.isBlank(jsonResult)) {
            ...
            if (StrUtil.isNotBlank(addresses)) {
                ...
                redisUtil.setEx(RedisConf.IP_SOURCE + RedisConf.SYMBOL_COLON + ip, addresses, 24, TimeUnit.HOURS);
            }
        } else {
